...
{
    private static readonly Dictionary<string, Action<OpenQA.Selenium.Interactions.Actions>> SpecialKeys =
    ...
    private static List<string> ParseInputString(string input)
    {
        ...
        while (i < input.Length)
        {
            ...
            if (c == '[')
            {
                var endIndex = input.IndexOf(']', i + 1);
                if (endIndex != -1)
                {
                    var potentialSpecialKey = input.Substring(i, endIndex - i + 1);
        ...
        if (currentSegment.Length > 0) segments.Add(currentSegment.ToString());
    ...
    {
        return new Dictionary<string, Action<OpenQA.Selenium.Interactions.Actions>>(
